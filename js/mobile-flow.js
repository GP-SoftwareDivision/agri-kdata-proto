...
var MCAP = /mcap=/.test(location.hash);   /* 모바일 화면설계서 캡처 모드 */
if(!M) return;   /* PC에는 아무 영향도 주지 않는다 */

/* ══════════ ① 전체 메뉴 (바둑판) ══════════ */
var AM_SVC = [
  {nav:'dashboard', n:'홈',         d:'시세 · 예측 · 오늘 할 일', bg:'var(--g100)',    ic:'<path d="M2.5 8.6L9 3L15.5 8.6"/><path d="M4.3 7.6V15.5H13.7V7.6"/><path d="M7.4 15.5V10.8H10.6V15.5"/>'},
...
function mdocLoadPlay(){
  ...
  function tick(){
    ...
    if(i < rows.length){
      ...
      i++; setTimeout(tick, (CAP||MCAP)?60:520);
    } else {
      mdoc.loaded = true;
...
(function(){
  var mm = location.hash.match(/mcap=([A-Z-]+)/);
  if(mm && MCAPS[mm[1]]){
    document.documentElement.classList.add('mcap');
    setTimeout(function(){ MCAPS[mm[1]](); }, 350);
  }
})();
